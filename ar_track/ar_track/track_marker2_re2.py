    ...
    try:    
        ...
        # 최신 상태 업데이트
            ...
            if node.dist is None:
                ...
                # 마커 위치 보정 (x 방향)
            if abs(node.pose.position.x) > 0.0025:
                if node.pose.position.x < -0.0025:
                    node.tw.angular.z = 0.075 * ANG_SPEED
                elif node.pose.position.x > 0.0025:
                    node.tw.angular.z = -0.075 * ANG_SPEED
            else:
                node.tw.angular.z = 0.0  # 정렬 완료 시 회전 중지

            # 거리 기반 전진/후진 제어
            if node.dist > dist_ref + margin:  # 너무 멀면 앞으로 이동
                node.tw.linear.x = 0.05
            elif node.dist < dist_ref - margin:  # 너무 가까우면 뒤로 이동
                node.tw.linear.x = -0.05
            else:  # 적정 거리면 정지
                node.tw.linear.x = 0.0
