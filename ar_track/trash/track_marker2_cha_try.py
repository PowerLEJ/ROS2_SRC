...
class TrackMarker(Node):
    ...
    def move_towards_marker(self):
        ...
        if self.marker_pose:
            # 마커와의 각도 계산
            angle_to_marker = atan2(self.marker_pose.position.y, self.marker_pose.position.x)
            distance_to_marker = sqrt(self.marker_pose.position.x**2 + self.marker_pose.position.y**2)
    ...
    def rotate_towards_marker(self, target_angle):
        """타겟 각도 방향으로 회전"""
        current_angle = self.twist.angular.z
